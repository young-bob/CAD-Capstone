        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            for (var round = 0; round < maxIterations; round++)
            {
                ...
                // Sanitize empty text blocks generated by Bedrock (e.g. Nova models) during tool calls
                // Bedrock validation will reject these empty blocks on the subsequent Converse request.
                if (assistantMessage.Content is not null)
                {
                    assistantMessage.Content = assistantMessage.Content
                        .Where(c => c.ToolUse is null ? !string.IsNullOrWhiteSpace(c.Text) : true)
                        .ToList();
                }
